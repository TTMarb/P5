...
# include <linux/efi.h>
#endif
void *my_xlate_dev_mem_ptr(unsigned long phys); //Lukas made this
void my_unxlate_dev_mem_ptr(unsigned long phys,void *addr); //Lukas
...
void *my_xlate_dev_mem_ptr(unsigned long phys)
{
	...
	// addr = (void __force *)ioremap_cache(start, PAGE_SIZE); Lukas
	dbgprint("Lukas try to call ioremap in my_xlate"); //Lukas
	if (addr)
...
void my_unxlate_dev_mem_ptr(unsigned long phys,void *addr)
{
	...
	// Not RAM, so it is some device (can be bios for example)
	// iounmap((void __iomem *)((unsigned long)addr & PAGE_MASK)); Lukas
